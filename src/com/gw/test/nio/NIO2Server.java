...
public class NIO2Server {
    ...
    public NIO2Server(int port) throws IOException {
        this.port = port;
    ...
    private class AcceptHandler implements CompletionHandler<AsynchronousSocketChannel, NIO2Server> {
        @Override
        public void completed(final AsynchronousSocketChannel client, NIO2Server attachment) {
            try {
            ...
            public void completed(Integer result, AsynchronousSocketChannel attachment) {
                try {
                    if (result < 0) {// 客户端关闭了连接
                        NIO2Server.close(attachment);
                    } else if (result == 0) {
                        System.out.println("空数据"); // 处理空数据
                    } else {
                        // 读取请求，处理客户端发送的数据
                        buffer.flip();
                        CharBuffer charBuffer = NIO2Server.decoder.decode(buffer);
    ...
    public static void main(String[] args) {
        try {
            ...
            NIO2Server server = new NIO2Server(PORT);
            server.listen();
        } catch (Exception e) {
            e.printStackTrace();
        }
